# This is Modified from https://github.com/facebookresearch/pal/blob/main/src/cot/data/data_processing.py 
"""
...
TOKEN_DICT = {
    ...
    "EoI": 62,
    "EoS": 63,
    "ANS": 64,  # NEW: Answer prompt token (like ":" in CODI's "The answer is:") 
    "BoT": 65,  # Beginning of the thought for latent COT in CODI 
    "EoT": 66,  # End of the thought for latent COT in CODI  
    "PAD": 67,  # PADDING is needed for CODI training 
}
...
class Polynomial_CODI(SequenceDataset):
    prefix = "polynomial"

    def __init__(self, mod=11, func=None, save_dir=None, cot=True, **kwargs):
        self.mod = mod
        assert cot == True, 'cot has to be true'
        if func is None:
            # polynomial iteration based on this function is not permutation invariant
            def func(x, y):
                return x * y + 1
    ...
    def generate_fixed_len_data(self, seq_len, n_data, rng=None):
        ...
        # allocate memory
        length = self.get_len(seq_len) + 1 # add 1 because we need to have additional ANS token. 
        data = np.empty((n_data, length), dtype=np.int32)
        ...
        # compute the sum 
        data[:, seq_len + 2 : -1] = data[:, 1 : seq_len + 1] # need to chane "seq + 2 :" to "seq + 2 : -1" because of the extra token added.  
        for t in range(1, seq_len):
            data[:, seq_len + 2 + t] = self.func(data[:, seq_len + 2 + t - 1], data[:, seq_len + 2 + t])
            data[:, seq_len + 2 + t] %= self.mod 

        data[:, -1] = data[:, -2]  
        data[:, -2] = TOKEN_DICT["ANS"] 
        return data
    ...
    def load_data(self, lengths, data_type=None):
        ...
        -----
        Should be called after `generate_datafiles`. 
        This method is overwritten because of additional ANS token. 
        """
        assert isinstance(lengths, list), "`lenghts` must be an a list of int."
        assert data_type in ["train", "test"], "`data_type` must be 'train' or 'test'."
        ...
        indices = np.cumsum(n_data_by_lens, dtype=int)
        indices = np.insert(indices, 0, 0)
        data = np.full((indices[-1], self.get_len(max(lengths)) + 2), TOKEN_DICT["EoS"], dtype=np.int32) # change self.get_len(max(lengths)) + 1 to self.get_len(max(lengths)) + 2. 

        # load the data in the allocated memory
        for i, seq_len in enumerate(lengths):
            data[indices[i] : indices[i + 1], : self.get_len(seq_len)+1] = np.load(
                self.save_dir / f"{data_type}_{seq_len}.npy"
            ) # change self.gen_len(seq_len) to self.gen_len(seq_len) + 1 
...
class HyperPolynomial(SequenceDataset):
    prefix = "hyper_polynomial"

    def __init__(self, mod=11, func=None, save_dir=None, cot=True, addition_value=1, **kwargs):
        self.mod = mod
        if func is None:

            # polynomial iteration based on this function is not permutation invariant
            def func(x, y):
                return x * y + addition_value
...
class HyperPolynomial_CODI(SequenceDataset):
    prefix = "hyper_polynomial"
...
def data_processing(
    ...
    **kwargs,
    ...
    ---------
    problem: str
        Problem to be solved. Currently supported are "binary-copy", "parity", "polynomial", "polynomial-hard", and "mix".
    n_len: int
