...
def preprocess_polynomial(
    ...
    for i in range(len(poly_codi)):
        ...
        answer_id = torch.tensor([POLYNOMIAL_TOKEN_DICT['EoT']] + answer_id.numpy().tolist(), dtype=torch.long)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        encoder_input_ids = torch.nn.utils.rnn.pad_sequence(reversed_input_ids, batch_first=True, padding_value= POLYNOMIAL_TOKEN_DICT['PAD'] if self.data_name in ["polynomial"] else self.tokenizer.pad_token_id).flip(1)
        
        # pad
        ref_input_ids = torch.nn.utils.rnn.pad_sequence(ref_input_ids, batch_first=True, padding_value= POLYNOMIAL_TOKEN_DICT['PAD'] if self.data_name in ["polynomial"] else  self.tokenizer.pad_token_id)
        ref_labels = torch.nn.utils.rnn.pad_sequence(ref_labels, batch_first=True, padding_value=IGNORE_INDEX) 

        decoder_input_ids = torch.nn.utils.rnn.pad_sequence(decoder_input_ids, batch_first=True, padding_value= POLYNOMIAL_TOKEN_DICT['PAD'] if self.data_name in ["polynomial"] else self.tokenizer.pad_token_id)
        ...
        return dict(
            ...
            labels=labels,
            encoder_attention_mask=encoder_input_ids.ne(POLYNOMIAL_TOKEN_DICT['PAD'] if self.data_name in ["polynomial"] else self.tokenizer.pad_token_id),
            ...
            ref_attention_mask=ref_input_ids.ne(POLYNOMIAL_TOKEN_DICT['PAD'] if self.data_name in ["polynomial"] else self.tokenizer.pad_token_id),
